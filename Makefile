# 프로젝트 설정
PROJECT_MODULE := services.shop
SETTINGS_MODULE := services.shop.shop.settings
...
.PHONY: test
test:
	python -m pytest --cov=services.shop

# coverage file 생성
coverage:
	python -m pytest --cov=services.shop --cov-report=html
...
.PHONY: bandit
bandit:
	python -m bandit -c bandit.yaml -r services.shop
